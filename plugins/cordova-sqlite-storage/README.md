...
- Drop-in replacement for HTML5/[Web SQL API](http://www.w3.org/TR/webdatabase/), the only change should be `sqlitePlugin.openDatabase()` --> `sqlitePlugin.openDatabase()`
...
- If you run into problems and your code follows the asynchronous HTML5/[Web SQL](http://www.w3.org/TR/webdatabase/) transaction API, you can try opening your database using `sqlitePlugin.openDatabase` and see if you get the same problems.
...
The idea is to emulate the HTML5/[Web SQL API](http://www.w3.org/TR/webdatabase/) as closely as possible. The only major change is to use `window.sqlitePlugin.openDatabase()` (or `sqlitePlugin.openDatabase()`) instead of `sqlitePlugin.openDatabase()`. If you see any other major change please report it, it is probably a bug.
...
db.transaction(function(tx) {
  tx.executeSql('CREATE TABLE IF NOT EXISTS recipes (id integer primary key, data text, data_num integer)');
  tx.executeSql('INSERT INTO recipes (data, data_num) VALUES (?,?)', ['test', 101], function(tx, res) {

  tx.executeSql('SELECT * FROM recipes;', [], function(tx, res) {
...
function onDeviceReady() {
  ...
  db.transaction(function(tx) {
    ...
    tx.executeSql("INSERT INTO recipes (data, data_num) VALUES (?,?)", ["test", 100], function(tx, res) {
      console.log("insertId: " + res.insertId + " -- probably 1");
      console.log("rowsAffected: " + res.rowsAffected + " -- should be 1");

      db.transaction(function(tx) {
        tx.executeSql("select count(id) as cnt from recipes;", [], function(tx, res) {
...
This case will also works with Safari (WebKit), assuming you replace `window.sqlitePlugin.openDatabase` with `sqlitePlugin.openDatabase`.
