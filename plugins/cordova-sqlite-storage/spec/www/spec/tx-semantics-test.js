      ...
      describe(suiteName + 'transaction callback semantics test(s)', function() {
        ...
        test_it(suiteName + "transaction encompasses all callbacks", function() {
          ...
          db.transaction(function(tx) {
            ...
            db.transaction(function(tx) {
              tx.executeSql('INSERT INTO recipes (data, data_num) VALUES (?,?)', ['test', 100], function(tx, res) {
                tx.executeSql("SELECT count(*) as cnt from recipes", [], function(tx, res) {
        ...
        test_it(suiteName + "error handler returning true causes rollback", function() {
          ...
          withTestTable(function(db) {
            db.transaction(function(tx) {
              tx.executeSql("insert into recipes (data, data_num) VALUES (?,?)", ['test', null], function(tx, res) {
                expect(res).toBeDefined();
                expect(res.rowsAffected).toBe(1);
        ...
        test_it(suiteName + "executeSql fails outside transaction", function() {
          withTestTable(function(db) {
            ...
            db.transaction(function(tx) {
              ...
              try {
                txg.executeSql("select count(*) as cnt from recipes", [], null, null);
        ...
        test_it(suiteName + "readTransaction should throw on modification", function() {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS recipes');
            tx.executeSql('CREATE TABLE IF NOT EXISTS recipes (foo text)');
            tx.executeSql('INSERT INTO recipes VALUES ("bar")');
          }, function () {}, function () {
            db.readTransaction(function (tx) {
              ...
              tasks = [
                ...
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('UPDATE recipes SET foo = "baz"');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('INSERT INTO recipes VALUES ("baz")');
                  }, checkDone, fail);
                },
                function () {
                  db.readTransaction(function (tx) {
                ...
                function () {
                  db.readTransaction(function (tx) {
                    tx.executeSql('CREATE TABLE recipes2');
                  }, checkDone, fail);
                }
              ];
