    ...
    describe(scenarioList[i] + ': tx value bindings test(s)', function() {
      ...
      describe(scenarioList[i] + ': tx column value insertion test(s)', function() {
        ...
        it(suiteName + "number values inserted using number bindings", function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS recipes');
            tx.executeSql('CREATE TABLE IF NOT EXISTS recipes (id integer primary key, data_text1, data_text2, data_int, data_real)');
          }, function(err) {
            expect(false).toBe(true);
            expect(err.message).toBe('---');
            ...
            db.transaction(function(tx) {
              ...
              tx.executeSql("insert into recipes (data_text1, data_text2, data_int, data_real) VALUES (?,?,?,?)", ["314159", "3.14159", 314159, 3.14159], function(tx, res) {
        ...
        it(suiteName + "executeSql parameter as array", function(done) {
          ...
          db.transaction(function(tx) {
            tx.executeSql('DROP TABLE IF EXISTS recipes');
            tx.executeSql('CREATE TABLE IF NOT EXISTS recipes (id integer primary key, data1, data2)');
          }, function(err) {
            expect(false).toBe(true);
            expect(err.message).toBe('---');
            ...
            db.transaction(function(tx) {
              // create columns with no type affinity
              tx.executeSql("insert into recipes (data1, data2) VALUES (?,?)", ['abc', [1,2,3]], function(tx, res) {
                expect(res).toBeDefined();
